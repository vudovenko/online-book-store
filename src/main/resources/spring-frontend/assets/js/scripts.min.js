                                    ...
                                    complete: function (result) {
                                        if (result.status === 200) {
                                            ...
                                            $Middle.find('.Middle-searchHeader').text('Найдено ' +
                                                ...
                                                (result.responseJSON.count % 10 < 10 && result.responseJSON.count % 10 > 20
                                                    ? (result.responseJSON.count % 10 === 1 ? 'a'
                                                        : (result.responseJSON.count % 10 > 1 && result.responseJSON.count % 10 < 5
                                                            ? 'и' : '')) : ''))
        ...
        var Card = function () {
            var bookTemplate = function (book) {
                return '<div class="Card">'
                    ...
                    (book.discount
                        ? '<div class="Card-sale">'
                            + parseInt(book.discount * 100) + '% скидка' +
                        '</div>' : '') +
                    (book.isBestseller
                    ...
                    (book.discount
                        ? '<span class="Card-priceOld">₽' + book.price
                        + '</span>\n' : '')
                    + '                       <span class="Card-price">' +
                    '₽' + (book.price * (1 - book.discount)).toFixed(2) + '</span>\n'
                    + '                </div>\n'
                    + '              </div>\n' + '            </div>';
            }
            return {
                        ...
                        switch (data) {
                            ...
                            case'recent':
                                getData('/api/books/recent', {
                                    from: $('[data-refreshfrom]').data('refreshfrom'),
                                    to: $('[data-refreshto]').data('refreshto'),
                                    offset: $this.data('refreshoffset') / $this.data('refreshlimit'),
                                    limit: $this.data('refreshlimit')
                                });
                                break;
                            case'popular':
                                getData('/api/books/popular', {
                                    from: $('[data-refreshfrom]').data('refreshfrom'),
                                    to: $('[data-refreshto]').data('refreshto'),
                                    offset: $this.data('refreshoffset') / $this.data('refreshlimit'),
                                    limit: $this.data('refreshlimit')
                                ...
                                if ($(slick.$slides[slick.$slides.length - 3]).hasClass('slick-active')) {
                                    if ($(slick.$slider).data('load')) {
                                        ...
                                        switch ($(slick.$slider).data('load')) {
                                            case'recommended':
                                                sendData('/api/books/recommended')
                                                break;
                                            case'recent':
                                                sendData('/api/books/recent')
                                                break;
                                            case'popular':
                                                sendData('/api/books/popular')
                                                break;
                                        }
                                    }
                                }
